...
@interface BMInfoViewController () <UIWebViewDelegate, UIScrollViewDelegate>
...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *versionViewTop;
@property (strong, nonatomic) NSString* htmlData;

@end

@implementation BMInfoViewController
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    float f = - self.webView.scrollView.contentOffset.y;
    if ( f > 108) f = 108;
    
    self.versionViewTop.constant = f;
    float phase = f/108.0;
    self.currentVersionLabel.alpha = (phase - 0.8)/(1.0 - 0.8);
    self.disclamerLabel.alpha = (phase - 0.3)/(0.5 - 0.3);
}
...
- (void)setVersionLabel {
    ...
    self.currentVersionLabel.text = [NSString stringWithFormat:@"Current version %@", version];
}

- (void)setWebViewText {
    self.webView.scrollView.delegate = self;
    [self.webView.scrollView setContentInset:UIEdgeInsetsMake(108, 0, 0, 0)];
    [self.webView.scrollView setScrollIndicatorInsets:UIEdgeInsetsMake(108, 0, 0, 0)];
